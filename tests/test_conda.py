...
# this package
import shippinglabel_conda.__main__  # noqa: F401
from shippinglabel_conda import (
		clear_cache,
...
def test_compile_requirements(tmp_pathplus: PathPlus) -> None:
...
def test_compile_requirements_markers_url_extras(tmp_pathplus: PathPlus) -> None:
	(tmp_pathplus / "requirements.txt").write_lines([
...
def test_prepare_requirements() -> None:
	requirements = [
			"apeye>=0.3.0",
			"click==7.1.2",
			"ruamel.yaml>=0.16.12",
...
def test_prepare_requirements_markers_url_extras(tmp_pathplus: PathPlus) -> None:
	requirements = [
			'apeye>=0.3.0; python_version <= "3.9"',
			"pip @ https://github.com/pypa/pip/archive/1.3.1.zip#sha1=da9234ee9982d4bbb3c72346a6de940a148ea686",
			'click==7.1.2; platform_system == "Linux"',
...
def test_get_channel_listing(should_clear_cache: Union[bool, str]) -> None:
	...
	if should_clear_cache:
		if isinstance(should_clear_cache, str):
...
def test_get_channel_listing_error() -> None:
...
def test_validate_requirements_unsatisfied() -> None:
	with pytest.raises(
			InvalidRequirement,
